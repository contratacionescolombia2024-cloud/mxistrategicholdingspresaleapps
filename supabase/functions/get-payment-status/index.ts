...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Step 4: Get payment status from NOWPayments
    console.log(`[${requestId}] Step 4: Getting payment status from NOWPayments...`);
    
    let nowPaymentsResponse;
    try {
      console.log(`[${requestId}] Calling NOWPayments API...`);
      nowPaymentsResponse = await fetch(`https://api.nowpayments.io/v1/payment/${payment_id}`, {
        method: 'GET',
        headers: {
